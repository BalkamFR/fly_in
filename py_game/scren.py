...
from parsing import Hub, ParsingFiles, pars_file


def drawing_function_menu(
...
class Screen:
    def __init__(
        ...
        self.control_drones: Optional[ControlDrone] = (
            setting_maps.control_drone if setting_maps else None
        )
        self.zoom = 2
        ...
        self.files_select = sorted(
            [f.name for f in self.path.iterdir() if f.is_file()]
        )
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.btn_quit.collidepoint(event.pos):
                        running = False
                    if (
                        hasattr(self, "map_buttons")
                        and page == "start_page"
                    ):
                        for file_name, rect in self.map_buttons:
                            if rect.collidepoint(event.pos):
                                try:
                                    self.setting_maps = pars_file(
                                        f"maps/{self.path_select}"
                                        f"/{file_name}"
                                    )
                                    self.error_message = ""
                    ...
                    if (
                        self.btn_easy.collidepoint(event.pos)
                        and page == "start_page"
                    ):
                        self.path_select = "easy"
                    ...
                    if (
                        self.btn_medium.collidepoint(event.pos)
                        and page == "start_page"
                    ):
                    ...
                    if (
                        self.btn_hard.collidepoint(event.pos)
                        and page == "start_page"
                    ):
                    ...
                    if (
                        self.btn_challenger.collidepoint(event.pos)
                        and page == "start_page"
                    ):
                    ...
                    if (
                        self.btn_start.collidepoint(event.pos)
                        and page == "start_page"
                    ):
    ...
    def all_transparent(
        ...
        border_color: Tuple[int, int, int] = 80, 130, 180
        i = 0
        for box in list_box:
            ...
            pygame.draw.rect(
                surface, color_font,
                surface.get_rect(), border_radius=radius,
            )
    ...
    def header(self) -> None:
        ...
        self.text_center_box(
            header_rect, "Fly-In", 70, self.first_color, "consolas"
        )
    ...
    def select_maps(
        ...
        for box in all_btn:
            self.box = pygame.Rect(
                start_x + padding_left, start_y + i,
                size_width, size_height,
            )
            self.all_transparent([self.box], True)
            self.text_left_box(
                self.box, f"{box}", 22, (225, 230, 240), "consolas"
            )

            self.map_buttons.append((box, self.box))
            i += padding
    ...
    def drawing_start_page(self) -> None:
        ...
        self.right_panel_rect = pygame.Rect(
            self.width - 400, (self.height - 835), 380, 280
        )
        ...
        self.text_center_box(
            self.btn_start, "Start", 30, (225, 230, 240), "consolas"
        )

        self.btn_quit = pygame.Rect(
            (self.width - 200), (self.height - 100), 180, 50
        )
        self.text_center_box(
            self.btn_quit, "Quit", 20, (225, 230, 240), "consolas"
        )

        self.left_panel_rect = pygame.Rect(
            15, self.height - 820, 480, 800
        )
        ...
        if len(self.error_message) == 0:
            self.text_center_box(
                self.detail_card, "Details carte", 26,
                (100, 200, 255), "consolas",
            )
            self.name_maps = pygame.Rect(
                self.width - 400, self.height - 770, 110, 70
            )
            self.text_left_box(
                self.name_maps, "Name: ", 18, (225, 230, 240), "consolas"
            )
            self.nb_drone_maps = pygame.Rect(
                self.width - 400, self.height - 730, 110, 70
            )
            self.text_left_box(
                self.nb_drone_maps, "Drones: ", 18,
                (225, 230, 240), "consolas",
            )
            self.hub_maps = pygame.Rect(
                self.width - 400, self.height - 690, 110, 70
            )
            self.text_left_box(
                self.hub_maps, "Hubs: ", 18, (225, 230, 240), "consolas"
            )
            self.mode_maps = pygame.Rect(
                self.width - 400, self.height - 650, 110, 70
            )
            self.text_left_box(
                self.mode_maps, "Mode: ", 18, (225, 230, 240), "consolas"
            )
            self.name_maps_text = pygame.Rect(
                self.width - 240, self.height - 770, 110, 70
            )
            self.hub_maps_text = pygame.Rect(
                self.width - 240, self.height - 690, 110, 70
            )
            self.nb_drone_maps_text = pygame.Rect(
                self.width - 240, self.height - 730, 110, 70
            )
            self.mode_maps_text = pygame.Rect(
                self.width - 240, self.height - 650, 110, 70
            )
            map_name = (
                self.setting_maps.name_file.split(".")[0]
                if self.setting_maps
                else ""
            )
            nb_drones = (
                str(self.setting_maps.nb_drone)
                if self.setting_maps
                else ""
            )
            nb_hubs = (
                str(len(self.setting_maps.all_name_hub))
                if self.setting_maps
                else ""
            )
            mode = self.path_select if self.setting_maps else ""
            self.text_left_box(
                self.name_maps_text, map_name, 15,
                (225, 230, 240), "consolas",
            )
            self.text_left_box(
                self.nb_drone_maps_text, nb_drones, 20,
                (225, 230, 240), "consolas",
            )
            self.text_left_box(
                self.hub_maps_text, nb_hubs, 20,
                (225, 230, 240), "consolas",
            )
            self.text_left_box(
                self.mode_maps_text, mode, 20,
                (225, 230, 240), "consolas",
            )
        else:
            self.text_center_box(
                self.detail_card, "Erreur parsing", 26,
                (255, 90, 90), "consolas",
            )
            ...
            for i, line in enumerate(lines):
                ...
                self.window.blit(
                    text_surf, (start_x, start_y + (i * 24))
                )
        ...
        self.btn_easy = pygame.Rect(
            marge + (0 * (size_btn + gap)), self.height - 800,
            size_btn, 50,
        )
        self.btn_medium = pygame.Rect(
            marge + (1 * (size_btn + gap)), self.height - 800,
            size_btn, 50,
        )
        self.btn_hard = pygame.Rect(
            marge + (2 * (size_btn + gap)), self.height - 800,
            size_btn, 50,
        )
        self.btn_challenger = pygame.Rect(
            marge + (3 * (size_btn + gap)), self.height - 800,
            size_btn, 50,
        )
        ...
        self.text_center_box(
            self.btn_easy, "Easy", 15, (80, 220, 130), "consolas"
        )
        self.text_center_box(
            self.btn_medium, "Medium", 15, (70, 140, 255), "consolas"
        )
        self.text_center_box(
            self.btn_hard, "Hard", 15, (235, 75, 85), "consolas"
        )
        self.text_center_box(
            self.btn_challenger, "Challenger", 15,
            (255, 195, 50), "consolas",
        )
    ...
    def draw_connections(self, size: int = 100) -> None:
        ...
        color_select: Tuple[int, int, int]
        if self.path_select == "easy":
        ...
        all_hubs: List[Hub] = (
            [start_hub, end_hub] + self.setting_maps.hub
        )
    ...
    def draw_all_drones(self) -> None:
        ...
        for drone in self.control_drones.all_drone:
            cx = int(
                self.hub_zone.x + margin + offset_x
                + (drone.pos_x - min_x) * scale
            )
            cy = int(
                self.hub_zone.y + margin + offset_y
                + (max_y - drone.pos_y) * scale
            )

            try:
                drone_img = pygame.image.load(drone.img_drone)
                drone_img = pygame.transform.scale(
                    drone_img, (size - 20, size - 20)
                )
                rect = drone_img.get_rect(center=(cx, cy))
                self.window.blit(drone_img, rect)
            except (pygame.error, FileNotFoundError):
                pygame.draw.circle(
                    self.window, (255, 75, 75), (cx, cy), 15
                )
    ...
    def drawing_hub_page(self) -> None:
        ...
        self.btn_home = pygame.Rect(
            self.width - 400, self.height - 100, 180, 50
        )
        surf = pygame.display.get_surface()

        temp_surf = pygame.Surface(
            (self.btn_home.width, self.btn_home.height), pygame.SRCALPHA
        )
        pygame.draw.rect(
            temp_surf,
            (35, 90, 200, 160),
            (0, 0, self.btn_home.width, self.btn_home.height),
            border_radius=15,
        )
        pygame.draw.rect(
            temp_surf,
        ...
        self.text_center_box(
            self.btn_home, "Home", 20, (225, 230, 240), "consolas"
        )
        ...
        if current_surface:
            for i, text in enumerate(info_lines):
                ...
                current_surface.blit(
                    text_surface, (start_x, start_y + (i * line_spacing))
                )

        self.all_transparent([self.btn_quit])
        self.all_transparent([self.hub_zone], flag=True)
        dh_start: Hub = self.setting_maps.start_hub
        dh_end: Hub = self.setting_maps.end_hub
        self.draw_connections()
        self.create_hub(dh_start)
        self.create_hub(dh_end)
